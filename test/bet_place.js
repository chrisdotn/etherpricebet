contract('Bet after placeBet()', function(accounts) {
    ...
    it('must not allow creating another bet');

    // it('must not allow to create another bet', function () {
    //     var bet = Bet.deployed();
    //     //FIXME throws, but test seems bogus
    //     assert.throws(function() {bet.create(priceLevel, {from:accounts[1], value:weiValue})}, Error, 'does not throw');
    // });
